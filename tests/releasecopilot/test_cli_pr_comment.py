    ...
    payload = {
        ...
        "files": {
            "src/releasecopilot/utils/coverage.py": {
                "summary": {"covered_lines": 44, "num_statements": 50}
            },
...
def test_pr_comment_dry_run(
    monkeypatch: pytest.MonkeyPatch, capsys: CaptureFixture[str], coverage_report: Path
) -> None:
    captured: dict[str, float] = {}

    def _fake_build(
        coverage: float, *, minimum: float = 70.0, paths: tuple[str, ...] | None = None
    ):
...
def test_pr_comment_infers_pr_from_event(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, coverage_report: Path
) -> None:
    posted: dict[str, object] = {}
    ...
    def _fake_request(method: str, url: str, token: str, data: dict | None = None):
        ...
        if method == "GET":
            return {
                "items": [
                    {
                        "body": cli_main.COMMENT_MARKER,
                        "url": "https://api.example.com/comment/1",
                    }
                ]
            }
        return {}
